...
{-
Elm's functional paradigm uses persistent data structures so "updating the model" returns a new model with only the changed data copied.
-}
type alias Model =
    { number : Int
    , password : String
...
update : Msg -> Model -> (Model, Msg) -- The reason we don't use a different notation to indicate the return has to do with currying. In a nutshell, currying means if you don't pass all the arguments to a function, another function will be returned that accepts whatever arguments are still needed.
update action model =
    case action of
        Inc ->
            (model + 2, action)
...
view model =
    div [ class "container" ]
        [ h1 [ class container, title "test"] [ text "Counterlicious" ]
        , div [] [ text "Count: ", text (toString (add model)) ]
        , button [ onClick Dec ] [ text "Decrement" ]
        , button [ onClick Inc ] [ text "Increment" ]
        , button [ onClick Sub ] [ text "Subtract" ]
        ]
--}
